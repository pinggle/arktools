...
void dumpClassDef(DexFile *pDexFile, int idx) {
    ...
    if (pClassData == NULL) {
        LOGE("Trouble reading class data\n");
        return;
    }
...
void dumpInstruction(DexFile *pDexFile, const DexCode *pCode, int insnIdx,
    ...
    LOGD("%06zx:", ((u1 *) insns - pDexFile->baseAddr) + insnIdx * 2);
    ...
    if (pDecInsn->opcode == OP_NOP) {
        ...
        if (instr == kPackedSwitchSignature) {
            LOGD("|%04x: packed-switch-data (%d units)",
                 insnIdx, insnWidth);
        } else if (instr == kSparseSwitchSignature) {
            LOGD("|%04x: sparse-switch-data (%d units)",
                 insnIdx, insnWidth);
        } else if (instr == kArrayDataSignature) {
            LOGD("|%04x: array-data (%d units)",
                 insnIdx, insnWidth);
        } else {
    ...
    switch (dexGetFormatFromOpcode(pDecInsn->opcode)) {
        ...
        case kFmt21t:        // op vAA, +BBBB
        {
            ...
            LOGD(" v%d, %04x // %c%04x", pDecInsn->vA,
                 insnIdx + targ,
                 (targ < 0) ? '-' : '+',
                 (targ < 0) ? -targ : targ);
        }
...
void dumpClass(DexFile *pDexFile, int idx, char **pLastPackage) {
    ...
    if (gOptions.exportsOnly && (pClassDef->accessFlags & ACC_PUBLIC) == 0) {
        //LOGD("<!-- omitting non-public class %s -->\n",
        //    classDescriptor);
        goto bail;
    }
    ...
    for (u4 i = 0; i < item->size; ++i) {
        ...
        switch ((MethodHandleType) mh.methodHandleType) {
            ...
            case MethodHandleType::INVOKE_INTERFACE:
                ...
                break;
            default:
                LOGD("Unknown method handle type 0x%02x, skipped.", mh.methodHandleType);
                continue;
        }
        ...
        if (is_invoke) {
            if (!getMethodInfo(pDexFile, mh.fieldOrMethodIdx, &info)) {
                LOGD("Unknown method handle target method@%04x, skipped.", mh.fieldOrMethodIdx);
                continue;
            }
        } else {
            if (!getFieldInfo(pDexFile, mh.fieldOrMethodIdx, &info)) {
                LOGD("Unknown method handle target field@%04x, skipped.", mh.fieldOrMethodIdx);
                continue;
            }
        }
        ...
        if (gOptions.outputFormat == OUTPUT_XML) {
            LOGD("<method_handle index index=\"%u\"\n", i);
            LOGD(" type=\"%s\"\n", type);
            LOGD(" target_class=\"%s\"\n", info.classDescriptor);
            LOGD(" target_member=\"%s\"\n", info.name);
            LOGD(" target_member_type=\"%c%s%s\"\n",
                 info.signature[0], instance, info.signature + 1);
            LOGD("</method_handle>\n");
        } else {
            LOGD("Method Handle #%u:\n", i);
            LOGD("  type        : %s\n", type);
            LOGD("  target      : %s %s\n", info.classDescriptor, info.name);
            LOGD("  target_type : %c%s%s\n", info.signature[0], instance, info.signature + 1);
        }
    }
}
        ...
        for (u4 i = 0; i < count; ++i) {
            ...
            switch (valueType) {
                ...
                case kDexAnnotationMethodType: {
                    ...
                    LOGD(doXml ? "type=\"MethodType\" value=\"(%s)%s\"/>" : "(%s)%s (MethodType)",
                         protoInfo.parameterTypes, protoInfo.returnType);
                    free(protoInfo.parameterTypes);
                    break;
                }
                case kDexAnnotationMethodHandle: {
                    u4 idx = (u4) readUnsignedLittleEndian(&data, valueArg + 1);
                    LOGD(doXml ? "type=\"MethodHandle\" value=\"%u\"/>" : "%u (MethodHandle)",
                         idx);
                    break;
                }
                case kDexAnnotationString: {
                    u4 idx = (u4) readUnsignedLittleEndian(&data, valueArg + 1);
                    LOGD(doXml ? "type=\"String\" value=\"%s\"/>" : "%s (String)",
                         dexStringById(pDexFile, idx));
                    break;
                }
                case kDexAnnotationType: {
                    u4 idx = (u4) readUnsignedLittleEndian(&data, valueArg + 1);
                    LOGD(doXml ? "type=\"Class\" value=\"%s\"/>" : "%s (Class)",
                         dexStringByTypeIdx(pDexFile, idx));
                    break;
                }
                case kDexAnnotationNull: {
                    LOGD(doXml ? "type=\"null\" value=\"null\"/>" : "null (null)");
                    break;
                }
                case kDexAnnotationBoolean: {
                    LOGD(doXml ? "type=\"boolean\" value=\"%s\"/>" : "%s (boolean)",
                         (valueArg & 1) == 0 ? "false" : "true");
                    break;
                }
                default:
                    // Other types are not anticipated being reached here.
                    LOGD("Unexpected type found, bailing on call site info.\n");
                    i = count;
                    break;
            }
            LOGD("\n");
...
void initOptions(const char *inputParam) {
