 ...
 */
int CHECK_LIST_SIZE(DexMapItem *_ptr, u4 _count, u4 _elemSize) {
...
int dexSwapAndVerify(u1 *addr, size_t len) {
     ...
     * one line code, map dex header to struct;
     * * */
    ...
    if (okay) {
        ...
        if (adler != expectedChecksum) {
            ALOGE("ERROR: bad checksum (%08lx, expected %08x)", adler, expectedChecksum);
            okay = false;
        }
    }
